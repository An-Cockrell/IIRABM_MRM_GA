...
void pmn::pmn_function(int pmnID){
	...
	if(wbc_migrate>0){
		...
		if(TNF+PAF>1){
			wbc_stick=IL1;
			ecArray[id].IL1ra=cytokineProductionRule(3,id,0,0);
		}
		if((wbc_stick>=1)&&(ecArray[id].ec_stick>=100)){
			wbc_migrate=max(float(0),cytokineProductionRule(4,id,0,0));}
		pmn_age--;
		if(pmn_age<0){
			x=xLoc;
			y=yLoc;
...
void mono::mono_function(int index){
	...
	ecArray[id].IL1ra=cytokineProductionRule(8,id,TNFr,IL_1r);
	...
	if(activation>0){
		...
		ecArray[id].IL8=cytokineProductionRule(13,id,TNFr,IL_1r);
		ecArray[id].IL12=cytokineProductionRule(14,id,TNFr,IL_1r);
		ecArray[id].IL10=cytokineProductionRule(15,id,TNFr,IL_1r);
		ecArray[id].IL1=cytokineProductionRule(16,id,TNFr,IL_1r);
		ecArray[id].TNF=cytokineProductionRule(17,id,TNFr,IL_1r);
	...
	if(activation<0){
		ecArray[id].IL10=cytokineProductionRule(18,id,TNFr,IL_1r);
	}
	if(wbc_migrate==1){
		heal(id);}
	if(wbc_roll==1){
...
//	cout<<"activation="<<activation<<"\n";
	...
	if(ecArray[id].IL12+ecArray[id].IL4>0){
		proTH1=cytokineProductionRule(20,id,0,0)*100;
		proTH2=cytokineProductionRule(21,id,0,0)*100;
...
void TH1::TH1function(int index){
	...
	if(ecArray[id].IL12>0){
		ecArray[id].IFNg=cytokineProductionRule(22,id,0,0);
	}
	wiggle(&orientation);
	move(orientation, &xLoc, &yLoc);
	TH1_age--;
...
void TH2::TH2function(int index){
	...
	if(ecArray[id].IL10>0){
		ecArray[id].IL4=cytokineProductionRule(23,id,0,0);
		ecArray[id].IL10=cytokineProductionRule(24,id,0,0);
	}
	wiggle(&orientation);
	move(orientation, &xLoc, &yLoc);
	TH2_age--;
