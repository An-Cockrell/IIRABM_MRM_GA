...
iparray=np.loadtxt('InternalParameterizationFull_Gen282.csv',delimiter=',')
if(rank==0):
    print("ParamShape=",iparray.shape)
myIP=iparray[rank,:]

myTNF,myIL4,myIL10,myGCSF,myIFNG,myFitness=getFitness(data,numStochasticReplicates,myIP)
print("FITNESS=",rank,myFitness)

if(rank==0):
    recvTNF=np.empty([data.shape[0]*numStochasticReplicates*size,15], dtype=np.float32)
    recvIL4=np.empty([data.shape[0]*numStochasticReplicates*size,15], dtype=np.float32)
    recvIL10=np.empty([data.shape[0]*numStochasticReplicates*size,15], dtype=np.float32)
    recvGCSF=np.empty([data.shape[0]*numStochasticReplicates*size,15], dtype=np.float32)
    recvIFNG=np.empty([data.shape[0]*numStochasticReplicates*size,15], dtype=np.float32)
else:
    recvTNF=None
    recvIL4=None
    recvIL10=None
...
if(rank==0):
    ...
    np.savetxt('TNF_Series_%srep.csv'%numStochasticReplicates,recvTNF,delimiter=',')
    np.savetxt('IL4_Series_%srep.csv'%numStochasticReplicates,recvIL4,delimiter=',')
    np.savetxt('IL10_Series_%srep.csv'%numStochasticReplicates,recvIL10,delimiter=',')
    np.savetxt('GCSF_Series_%srep.csv'%numStochasticReplicates,recvGCSF,delimiter=',')
    np.savetxt('IFNG_Series_%srep.csv'%numStochasticReplicates,recvIFNG,delimiter=',')
