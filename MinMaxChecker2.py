...
def checker(filename,filename2,tnfArray,il4Array,il10Array,gcsfArray,ifngArray,totalCount,minarray,maxarray):
    ...
    for k in range(15):
        if(x[0,k]<(tnfMaxs[k]+tolerance) and
            ...
            x[4,k]<(ifngMaxs[k]+tolerance) and
            y[0,k]>(tnfMins[k]-tolerance) and
            y[1,k]>(il4Mins[k]-tolerance) and
            y[2,k]>(il10Mins[k]-tolerance) and
            y[3,k]>(gcsfMins[k]-tolerance) and
            y[4,k]>(ifngMins[k]-tolerance)):
            counter=counter+1
    if(counter>=countTol):
        for k in range(15):
...
np.save('TnfArrayFull.npy',tnfArray)
np.save('IL4ArrayFull.npy',il4Array)
np.save('IL10ArrayFull.npy',il10Array)
np.save('GCSFArrayFull.npy',gcsfArray)
np.save('IFNGArrayFull.npy',ifngArray)

np.savetxt('MinArrayFull.csv',minArray,delimiter=',')
np.savetxt('MaxArrayFull.csv',maxArray,delimiter=',')
