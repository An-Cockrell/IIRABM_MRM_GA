...
for k in range(numOuterIters):
    for i in range(numIters):
        ...
        mnFile=str('MinMax/Mins_Pos_%s_%s_%s_%s.csv'%(runID,k,i,rank))
        mxFile=str('MinMax/Maxs_Pos_%s_%s_%s_%s.csv'%(runID,k,i,rank))
        ...
        if(rank==0):
            fnamev=str('NumViable_Pos_IS%s_Gen%s_%s_%s.csv'%(injSize,k,i,runID))
            np.savetxt(fnamev,iparray,delimiter=',')

        if(rank==0):
            iname=str('InternalParameterization_Pos_IS%s_Gen%s_%s_%s.csv'%(injSize,k,i,runID))
            fname=str('Fitness_Pos_IS%s_Gen%s_%s_%s.csv'%(injSize,k,i,runID))
