...
il4Maxs=np.array([0.64703120,0.480755551,0.421347298,0.392763765,0.36483923,0.33320727,0.299190185,0.264745459,0.23142543])
...
for k in range(numOuterIters):
    for i in range(numIters):
        ...
        if(rank==0):
            mnFile=str('MinMax/MinsNorm_Pos_%s_%s.csv'%(k,i))
            mxFile=str('MinMax/MaxsNorm_Pos_%s_%s.csv'%(k,i))
        ...
        if(rank==0):
            fnamev=str('NumViableNorm_Pos_IS%s_Gen%s_%s.csv'%(injSize,k,i))
            np.savetxt(fnamev,recvbufV,delimiter=',')

        if(rank==0):
            iname=str('InternalParameterizationNorm_Pos_IS%s_Gen%s_%s.csv'%(injSize,k,i))
            fname=str('FitnessNorm_Pos_IS%s_Gen%s_%s.csv'%(injSize,k,i))
