...
#    print(rank,ifngResult)
    ...
    return tnfResult,fitsum

iparray=np.loadtxt('InternalParameterization_Gen89.csv',delimiter=',')
if(rank==0):
    print("ParamShape=",iparray.shape)
myIP=iparray[rank,:]

myTNF,myFitness=getFitness(data,numStochasticReplicates,myIP)
print("FITNESS=",rank,myFitness)

count=np.zeros(size,dtype=np.int)
disp=np.zeros(size,dtype=np.int)
for i in range(size):
#    count[i]=numMatrixElements
    disp[i]=15*i
...
#print(rank,myTNF.shape,myTNF)
comm.Barrier()
# comm.Gatherv(myTNF,[recvTNF,count,disp,MPI.FLOAT], root=0)
# if(rank==0):
#     print("Gather 1")
comm.Gather(myTNF,recvTNF, root=0)
if(rank==0):
    print("Gather 1")

if(rank==0):
    np.savetxt('bTNF_Series_%srep.csv'%numStochasticReplicates,recvTNF,delimiter=',')
